...
public class YandexTest {
    ...
    private void setUp() {
        ...
        page.go();
        page.authorization().
                waitUntil("ожидание окошка авторизации", DisplayedMatcher.displayed(), 5);
        page.authorization().authorizationButton().click();
        page.passLoginForm().
                waitUntil("ожидание окошка ввода логина", DisplayedMatcher.displayed(), 5);
        ...
        page.passLoginForm().passField().
                waitUntil("ожидание окошка ввода пароля", DisplayedMatcher.displayed(), 5);
    ...
    private void goToSettings() {
        ...
        page.popUpmenu().
                waitUntil("ожидание", DisplayedMatcher.displayed(), 5);
        page.popUpmenu().allSettings().click();
        page.langSettings().waitUntil("переход в настройки", DisplayedMatcher.displayed(), 5);
    ...
    private void languageSwitch(Language val) {
        ...
        if (currentLang.equals(val.getLanguageName())) {
            ...
            page.langDropDownBox().waitUntil("выбор языка", DisplayedMatcher.displayed(), 5);
    ...
    private void languageSwitchCheck(Language selected) {
        page.langDropDownBox().waitUntil("пока не изчезнет", not(DisplayedMatcher.displayed()), 5);
        Assert.assertTrue(page.langSettings().changeLangButton()
                .getText().equals(selected.getLanguageName()));
    }
    ...
    private void mailSendCheck() {
        if (page.mailComposeField().mailErrorMessage().isDisplayed()) {
            ...
            page.mailDonePage().messageDoneTitle().
                    waitUntil("успешная отпрвка", DisplayedMatcher.displayed(), 5);
    ...
    private void delMessages() {
        ...
        previously = Integer.parseInt(page.messagesTypes().incomingMessages().getText().replaceAll("[^0-9]+", ""));
    }

    @Step("выделение писем {address}")
    ...
    private void deleteMessagesCheck() {
        ...
        if (dell == 0) {
            ...
            page.messagesTypes().waitUntil("обновление количества писем", DisplayedMatcher.displayed(), 5);
            int actual = Integer.parseInt(page.messagesTypes().incomingMessages().getText().replaceAll("[^0-9]+", ""));
